...
class WAE(object):
    ...
    def train(self, data):
        ...
        for epoch in range(opts["epoch_num"]):
            ...
            for it in range(int(batches_num)):
...
def save_plots(opts, sample_train, sample_test,
    ...
    y = np.log(np.abs(losses_rec[::int(x_step)]))
    plt.plot(x, y, linewidth=2, color='red', label='log(|rec loss|)')

    y = np.log(np.abs(losses_match[::int(x_step)]))
    ...
    if len(encoding_changes) > 0:
        ...
        x_step = len(x) / len(y)
        plt.plot(x[::int(x_step)], y, linewidth=2, color='green', label='log(encoding changes)')
    plt.grid(axis='y')
